...
namespace SamanthaDJ.ServiceWPF.Tools {
    ...
    public class Log {
        ...
        public static void SamanthaService(string origin, string message) {
            WriteData("SamanthaService", origin, message);
        }
        ...
        private static void WriteData(string id, string origin, string message) {
            ...
            while (tryWrite) {
                try {
                    string nameDirectoryYearMonth = Global.PathLogOut + "\\" + DateTime.Now.ToString("yyyy-MM");
